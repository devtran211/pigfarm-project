...
async function calculateAndSaveInvestmentCost({ barns, toDeductFood = [], toDeductMed = [], session }) {
  ...
  for (const barn of barns) {
    ...
    if (existing) {
      existing.food_cost = Number(existing.food_cost) + food_cost;
      existing.medition_cost = Number(existing.medition_cost) + medition_cost;
      existing.total = existing.food_cost + existing.medition_cost + existing.breeding_cost;
...
async function calculateAndSaveInvestmentUpdateCost({ barns, toDeductFood = [], toDeductMed = [], session }) {
  // total pigs
  const totalPigs = barns.reduce((s, b) => s + (Number(b.total_pigs) || 0), 0);
  const equalShare = (barns.length > 0) ? (1 / barns.length) : 0;
...
async function calculateAndSaveInvestmentUpdateMedCost({ barns, toDeductMed = [], session }) {
  // total pigs
  const totalPigs = barns.reduce((s, b) => s + (Number(b.total_pigs) || 0), 0);
  const equalShare = (barns.length > 0) ? (1 / barns.length) : 0;
...
async function calculateAndSavePigImportCost(barn, herd, pigNumber){
  const originalInventory = herd.originalInventory;
  const importPrice =  herd.importPrice;

  const price = importPrice / originalInventory;
  const pigPrice = pigNumber * price;
  const total = pigPrice;

  const existing = await InvestmentCostModel.findOne({barn: barn._id});
  if(existing){
      existing.breeding_cost = Number(pigPrice);
      existing.total = existing.food_cost + existing.medition_cost + existing.breeding_cost;
      await existing.save();
  } else {
    await InvestmentCostModel.create([{
        breeding_cost: pigPrice ,
        food_cost: 0,
        medition_cost: 0,
        total,
        barn: barn._id
...
async function calculateAndSavePigUpdateCost(barn, herd, pigNumber){

  if(pigNumber < 0) pigNumber *= -1;
  ...
  const existing = await InvestmentCostModel.findOne({barn: barn._id});
  if(existing){
      console.log(existing.breeding_cost);
      existing.breeding_cost = Number(existing.breeding_cost) - Number(pigPrice);
      console.log(existing.breeding_cost);
      existing.total = existing.food_cost + existing.medition_cost + existing.breeding_cost;
      await existing.save();
  } 
...
async function calculateAndSavePigSplitCost(barn, herd, pigNumber){
...
module.exports = { 
  calculateAndSaveInvestmentCost, 
  calculateAndSaveInvestmentUpdateCost, 
  calculateAndSaveInvestmentMedCost, 
  calculateAndSaveInvestmentUpdateMedCost,
  calculateAndSavePigImportCost, 
  calculateAndSavePigUpdateCost,
  calculateAndSavePigSplitCost
};